...
/**
 * @brief initialize specified light
 * @param[in] light_type: specified light type
 */
...
#if LIGHT_FN_ENABLE
/**
 * @brief initialize light friendship config
 * @param[in] fn_num: amount of lpn
 * @param[in] queue_size: length of each message queue
 * @param[in] frnd_rx_window: rx window length
 */
static bool light_fn_init(uint8_t fn_num, uint8_t queue_size, uint8_t frnd_rx_window)
{
    fn_params_t fn_params;
    fn_params.queue_size = queue_size;
    if (frnd_rx_window > 0 && frnd_rx_window < 0xff)
    {
        mesh_node.frnd_rx_window = frnd_rx_window;
    }
    mesh_node.frnd_tx_ahead = 10;
    mesh_node.frnd_upd_times = 8;
    return fn_init(fn_num, &fn_params, fn_cb);;
}
#endif
