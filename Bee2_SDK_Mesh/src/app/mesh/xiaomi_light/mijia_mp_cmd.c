...
static void mijia_mp_cmd_check_dev_info(uint16_t opcode, const uint8_t *data, uint16_t len)
{
    ...
    dev_info.version[0] = MIBLE_LIB_MAJOR;
    dev_info.version[1] = MIBLE_LIB_MINOR;
    dev_info.version[2] = MIBLE_LIB_REVISION;
    dev_info.version[3] = (uint16_t)atoi(_VERSION(DEVELOPER_VERSION));
    dev_info.version[4] = (uint16_t)atoi(_VERSION(DEVELOPER_VERSION))>>8;
