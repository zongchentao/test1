...
extern mesh_model_info_t generic_on_off_server_1st;
 ...
 * @return command process status
 */
static user_cmd_parse_result_t user_cmd_mi_otp_write_mac(user_cmd_parse_value_t *pparse_value)
{
    /* update mac address */
    if (!UpdateMAC((uint8_t *)dev_address))
    {
        data_uart_debug("write mac address failed\r\n");
    }
    else
    {
        data_uart_debug("write mac address success, restart needed\r\n");
    }
    return USER_CMD_RESULT_OK;
}

/**
 * @brief simulate write certificates
 * @param[in] pparse_value: command parameters
 * @return command process status
 */
static user_cmd_parse_result_t user_cmd_mi_otp_simulate_write(user_cmd_parse_value_t *pparse_value)
{
    int ret = mi_mesh_otp_program_simulation();
    data_uart_debug("simulate write otp: %d\r\n", ret);
    return USER_CMD_RESULT_OK;
}
...
/**
 * @brief dump certificates
 * @param[in] pparse_value: command parameters
 * @return command process status
 */
static user_cmd_parse_result_t user_cmd_mi_otp_dump(user_cmd_parse_value_t *pparse_value)
{
    return USER_CMD_RESULT_OK;
}

/**
 * @brief erase certificates region
 * @param[in] pparse_value: command parameters
 * @return command process status
 */
static user_cmd_parse_result_t user_cmd_mi_otp_erase(user_cmd_parse_value_t *pparse_value)
{
    uint32_t sector_addr = (uint32_t)POTP_BASE;
    sector_addr &= ~(FLASH_SECTOR_SIZE - 1);
    /* erase otp */
    bool flag = flash_erase_locked(FLASH_ERASE_SECTOR, sector_addr);
    if (flag)
    {
        data_uart_debug("erase otp: success(0x%x)\r\n", sector_addr);
    }
    else
    {
        data_uart_debug("erase otp: failed(0x%x)\r\n", sector_addr);
    }
...
//    generic_on_off_publish(&generic_on_off_server_1st, (generic_on_off_t)onoff);
...
const user_cmd_table_entry_t mi_cmd_table[] =
{
    ...
    },
    {
        ...
        "miotpwm\r\n",
        "write mac address\r\n",
        user_cmd_mi_otp_write_mac
    },
    {
        "miotpsw",
        "miotpsw\r\n",
        "simulate write mi otp\r\n",
        user_cmd_mi_otp_simulate_write
    },
    {
        "miotpv",
        "miotpv\r\n",
    ...
    },
    {
        ...
        "miotpd\r\n",
        "dump mi otp\r\n",
        user_cmd_mi_otp_dump
    },
    {
        "miotpe",
        "miotpe\r\n",
        "erase mi otp\r\n",
        user_cmd_mi_otp_erase
    },
    {
        "midfue",
        "midfue\r\n",
