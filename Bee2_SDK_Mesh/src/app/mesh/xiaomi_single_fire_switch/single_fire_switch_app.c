...
static void switch_adjust_priority(void)
{
    ...
    pri_param.adv_priority.priority_level = GAP_VENDOR_PRIORITY_LEVEL_1;
    ...
    case GAP_MSG_LE_DEV_STATE_CHANGE:
        {
            if (start_up)
            {
                switch_adjust_priority();
                if (mesh_node.node_state == PROV_NODE)
                {
                    switch_timer_start();
                    switch_periodic_pub_stop();
                    mibeacon_adv_stop();
                }
                else
                {
