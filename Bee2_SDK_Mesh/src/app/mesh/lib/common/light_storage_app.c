...
static bool light_state_restore(void)
{
    ...
    if (node_state == PROV_NODE)
    {
        ...
        ret = light_flash_read(LIGHT_FLASH_PARAM_TYPE_LIGHT_STATE, sizeof(light_flash_light_state_t),
                               &light_state);
        light_flash_light_on_state_t on_state;
        ret = light_flash_read(LIGHT_FLASH_PARAM_TYPE_LIGHT_ON_STATE, sizeof(light_flash_light_on_state_t),
                               &on_state);
        ...
        {
            light_get_green()->lightness = 0;
            light_get_green()->lightness_last = light_state.state[3];
            light_get_blue()->lightness = 0;
            light_get_blue()->lightness_last = light_state.state[4];
        }
...
bool light_on_state_store(bool on)
{
    light_flash_light_on_state_t on_state = {on, {0, 0, 0}};
    return light_flash_write(LIGHT_FLASH_PARAM_TYPE_LIGHT_ON_STATE,
                             sizeof(light_flash_light_on_state_t),
                             &on_state);
}
