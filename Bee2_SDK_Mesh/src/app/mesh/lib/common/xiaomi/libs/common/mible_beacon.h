...
#if !defined(CONTROLLER_FASTFAIR) || (CONTROLLER_FASTFAIR == 0)
/**
...
*/
int set_fastpair_timeout(uint16_t time_ms);

/**
*@brief   .force the fastrpair stopped immediately
*@param    none
*/
int set_fastpair_force_stopped(void);

/**
*@brief    button object set timeout to stop adv
*@param    [in]time_ms: unit-ms
*/
int set_object_timeout(uint16_t time_ms);
#endif
...
uint32_t mibeacon_get_seqnum(void);

void mibeacon_set_object_power_saving(uint8_t power_saving_bitmask);
...
int mibeacon_period_spec_property_add(uint8_t siid, uint16_t piid);

int mibeacon_period_spec_event_add(uint8_t siid, uint16_t eiid);

/**
 *@brief    enable or disable period adv
 *@param    [in]enable: 1-switch on; 0-switch off
 */
mible_status_t period_obj_adv_enable(uint8_t enable);
#endif
