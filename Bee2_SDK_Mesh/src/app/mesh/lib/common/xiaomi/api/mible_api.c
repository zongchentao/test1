...
#ifndef MIBLE_SCENE_REC_ID_BASE
#define MIBLE_SCENE_REC_ID_BASE     0x80
#endif
...
/**
 * @brief   Set utc systime and start sys timer.
 * @param   [in] utc_time: time scine 1970/1/1
 * @return  MI_SUCCESS             If the timer was successfully started.
 *          MI_ERR_RESOURCES       Timer creat fail.
 *
 * */
__WEAK mible_status_t mible_systime_utc_set(uint32_t utc_time)
{
    return MI_SUCCESS;
}
...
/**
 * @brief   Set utc systime and start sys timer.
 * @return  utctime: time scine 1970/1/1.
 *
 * */
__WEAK uint32_t mible_systime_utc_get(void)
{
    return 0;
}
 ...
 *          MI_ERR_INVALID_PARAMS   Invalid record id supplied.
 * */
__WEAK mible_status_t mible_scene_delete(uint16_t record_id)
{
    return mible_record_delete(MIBLE_SCENE_REC_ID_BASE+record_id);
}
 ...
 *          MI_ERR_INVALID_ADDR     Invalid pointer supplied.
 * */
__WEAK mible_status_t mible_scene_read(uint16_t record_id, uint8_t* p_data,
        uint8_t len)
{
    return mible_record_read(MIBLE_SCENE_REC_ID_BASE+record_id, p_data, len);
}
 ...
 *          When record writing complete , call mible_arch_event_callback function and pass MIBLE_ARCH_EVT_RECORD_WRITE_CMP event and result. 
 * */
__WEAK mible_status_t mible_scene_write(uint16_t record_id, const uint8_t* p_data,
        uint8_t len)
{
    return mible_record_write(MIBLE_SCENE_REC_ID_BASE+record_id, p_data, len);
}
