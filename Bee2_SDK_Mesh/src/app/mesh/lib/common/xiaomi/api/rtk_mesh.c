...
{
    if(pargs == NULL || pargs->ttl <= 1)
        return MESH_MSG_SEND_CAUSE_INVALID_ACCESS_PARAMETER;
    pargs->ttl -= 1;
...
static bool cfg_server_receive_peek(mesh_msg_p pmesh_msg)
{
    ...
    case MIBLE_MESH_MSG_CONFIG_RELAY_SET:
    {
        ...
        break;
    case MIBLE_MESH_MSG_CONFIG_NETWORK_TRANSMIT_SET:
    {
        cfg_net_transmit_set_t *pmsg = (cfg_net_transmit_set_t *)pbuffer;
        evt_vendor_param.config_msg.nettx_set.net_retrans_cnt = pmsg->count;
        evt_vendor_param.config_msg.nettx_set.net_retrans_intvlsteps = pmsg->steps;
        mible_mesh_event_callback(MIBLE_MESH_EVENT_CONFIG_MESSAGE_CB, &evt_vendor_param);
        ret = cfg_server_receive(pmesh_msg);
    }
        break;
    case MIBLE_MESH_MSG_CONFIG_DEFAULT_TTL_SET:
    {
        cfg_default_ttl_set_t *pmsg = (cfg_default_ttl_set_t *)pbuffer;
        evt_vendor_param.config_msg.ttl_set.ttl = pmsg->ttl;
        mible_mesh_event_callback(MIBLE_MESH_EVENT_CONFIG_MESSAGE_CB, &evt_vendor_param);
        ret = cfg_server_receive(pmesh_msg);
    }
        break;
...
int mible_mesh_device_init_stack(void)
{
    ...
    mesh_node_cfg_t node_cfg =
    {
        ...
        .vir_addr_num = 3,
        .rpl_num = MI_RPL_SIZE,
        .sub_addr_num = MI_DEFAULT_GROUP_ADDR_NUM,
        .proxy_num = 1,
        .udb_interval = 5,
        .snb_interval = 300,    /** 30s snb */
        .prov_interval = 10,
        .proxy_interval = 10,
    ...
    /** init iv update callback */
    iv_index_cb_reg(iv_index_cb);
...
int mible_mesh_device_set_nettx(uint8_t ttl, uint8_t count, uint8_t interval)
{
    mesh_node.ttl = ttl;
    mesh_node.net_trans_count = count;
    mesh_node.net_trans_steps = interval;
    return 0;
}
...
int mible_mesh_device_get_nettx(uint8_t *ttl, uint8_t *count, uint8_t *step)
{
    if(ttl != NULL)
        *ttl = mesh_node.ttl;
    if(count != NULL)
        *count = mesh_node.net_trans_count;
    if(step != NULL)
        *step = mesh_node.net_trans_steps;
    //MI_LOG_WARNING("relay\t stat %d\t cnt %d\t interval %d\n", *enabled, *count, (*step+1)*10);
    return 0;
}
...
{
//	MI_LOG_DEBUG("mible_mesh_device_snb_start enable %d, mesh_node.features.snb=%d, mesh_node.snb_interval=%d\n", enable, mesh_node.features.snb, mesh_node.snb_interval);
    if(enable){
//        if (mesh_node.features.snb)   //when call beacon_stop(),mesh_node.features.snb is set to 0.
		if(is_provisioned)
            beacon_start();
    }else{
        beacon_stop();
...
{
    if(op == MIBLE_MESH_OP_ADD){
        mesh_model_sub(miot_server.pmodel, sub_addr);
#if defined(MI_MESH_TEMPLATE_TWO_KEY_SWITCH) || defined(MI_MESH_TEMPLATE_THREE_KEY_SWITCH)
        mesh_model_sub(generic_on_off_server_2st.pmodel, sub_addr);
    #if defined(MI_MESH_TEMPLATE_THREE_KEY_SWITCH)
        mesh_model_sub(generic_on_off_server_3rd.pmodel, sub_addr);
    #endif
#endif
#if defined(MI_MESH_TEMPLATE_LIGHTCTL)
        mesh_model_sub(light_ctl_temperature_server.pmodel, sub_addr);
#endif
    }else{
        mesh_model_unsub(miot_server.pmodel, sub_addr);
#if defined(MI_MESH_TEMPLATE_TWO_KEY_SWITCH) || defined(MI_MESH_TEMPLATE_THREE_KEY_SWITCH)
        mesh_model_unsub(generic_on_off_server_2st.pmodel, sub_addr);
    #if defined(MI_MESH_TEMPLATE_THREE_KEY_SWITCH)
        mesh_model_unsub(generic_on_off_server_3rd.pmodel, sub_addr);
    #endif
#endif
#if defined(MI_MESH_TEMPLATE_LIGHTCTL)
        mesh_model_unsub(light_ctl_temperature_server.pmodel, sub_addr);
#endif
    }
    return 0;
}
...
int mible_mesh_device_set_sub_address(mible_mesh_op_t op, uint16_t element, uint16_t company_id, uint16_t model_id, uint16_t sub_addr)
{
    ...
    // unsub pre_sub_addr
    for(int i=0; i<sizeof(pre_sub_addr)/2; i++){
        mesh_model_unsub(pmodel, pre_sub_addr[i]);
    }

    if(op == MIBLE_MESH_OP_ADD){
        MI_LOG_DEBUG("mi_sub_address: sub model(0x%08x-%d) group address(%d)", model_id, element, sub_addr);
        if(mesh_model_sub(pmodel, sub_addr) == false){
            mesh_model_sub_clear(pmodel);
        ...
        MI_LOG_DEBUG("mi_sub_address: overwrite model(0x%08x-%d) group address(%d)", model_id, element, sub_addr);
        mesh_model_sub_clear(pmodel);
        mesh_model_sub(pmodel, sub_addr);
    }else if(op == MIBLE_MESH_OP_DELETE_ALL){
        MI_LOG_DEBUG("mi_sub_address: clear model(0x%08x-%d)", model_id, element);
        mesh_model_sub_clear(pmodel);
    }
    mesh_flash_store(MESH_FLASH_PARAMS_MODEL_SUBSCRIBE_ADDR, pmodel);
    // resub pre_sub_addr
    for(int i=0; i<sizeof(pre_sub_addr)/2; i++){
        mesh_model_sub(pmodel, pre_sub_addr[i]);
    }
...
{
    if(addr != NULL){
        mesh_model_sub_dump_internal((mesh_model_p)miot_server.pmodel, addr, max_count);
    }
    return 0;
}
...
int mible_mesh_node_generic_control(mible_mesh_access_message_t *param)
{
    ...
    if(param->buf_len > 8)
        memcpy(msg + ACCESS_OPCODE_SIZE(opcode), param->p_buf, param->buf_len);
    else
        memcpy(msg + ACCESS_OPCODE_SIZE(opcode), param->buff, param->buf_len);
