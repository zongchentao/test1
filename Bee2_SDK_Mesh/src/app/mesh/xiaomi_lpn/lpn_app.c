...
#if MI_MANU_TEST_ENABLE
            mijia_mp_cmd_parse(&data, sizeof(data));
#endif

#if MI_USER_CMD_ENABLE
//            user_cmd_collect(&data, sizeof(data), mi_cmd_table);
#endif
        }
        break;
    ...
    case IO_MSG_TYPE_TIMER:
        light_sw_timer_handle_timeout(&io_msg);
        break;
    case IO_MSG_TYPE_GPIO:
        if (io_msg.subtype == MSG_BUTTON_SHORT_PRESS)
        {
            ...
            else if (mesh_node.node_state == PROV_NODE)
            {
                if (!is_fn_exist)
                {
                    start_lpn_req();
        ...
        else if (io_msg.subtype == MSG_BUTTON_LONG_PRESS)
        {
            start_beacon_timer();
            if (mesh_node.node_state == UNPROV_DEVICE)
            {
    ...
    case LPN_CB_TYPE_ESTABLISH_SUCCESS:
        {
            ...
            allowEnterDlps = true;
            is_fn_exist = true;
            printi("established with fn 0x%04x", fn_addr);
        }
        break;
