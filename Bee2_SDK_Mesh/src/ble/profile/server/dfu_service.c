...
#pragma diag_warning=Pa039
    ...
    case DFU_OPCODE_REPORT_TARGET_INFO://0x06
        if (length == DFU_LENGTH_REPORT_TARGET_INFO)
        {
            ...
            if (g_dfu_para.signature == SIGNATURE_APP_DEFINE)
            {
                T_IMG_HEADER_FORMAT *p_appdefine_header = (T_IMG_HEADER_FORMAT *)APP_DEFINE_DATA_ADDR;
                g_dfu_para.origin_image_version = p_appdefine_header->git_ver.ver_info.version;
            }
            else
            {
